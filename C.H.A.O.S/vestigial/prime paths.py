...
for x in range(1, 5):
    ...
    if invalid == 0 and x not in primes:
...
def forge(front, back, path):
    ...
    if len(path[fb][1]) == 0 and len(path[fb][2]) == 0 and len(path[fb][3]) == 0:
        ...
        for x in range(4):
            ...
            route = stream(front, back, front_row, back_row, max_steps, x)
...
for p in paths:
    ...
    for a in paths[p]:
        ...
        if len(paths[p][a]) > 0:
            ...
            paths[p][a] = sorted(paths[p][a], key=lambda x:x[-1])
            ...
            for t in paths[p][a]:
                ...
                f = t[0] * 2
                n = t[1] * 2
                d_rule = rule_gen(t[2], base, length)[0]
                step = 0

                row = rule_gen(f, base, length)[1]

                while step < t[-1]:

                    row = Color_cells(d_rule, length, row)[0]

                    step += 1

                if f == decimal(row, base):
                    print(t)
                    print(f)
                    print(decimal(row, base))
