...
def Chaos_Window(base, cell_vel, analytics, device_id=-1):
    ...
    if base < 5:
        ...
        value_color = {0:black, 1:magenta, 2:cyan, 3:yellow}
    ...
    def redraw_window(input_box, v_input, zero_count, step_show, triggers, dt, timer):
        ...
        # print(brush_height)
        #
        brush = pygame.Rect(glove_pos, (2, 2))
        pygame.draw.rect(WIN, (255, 255, 255), brush)
    ...
    momentum_step = 3
    momentum_scale_scale = 2
    stream_vel = {0:1, 1:1, 2:1, 3:1}
    stream_vel_scale = 40
    #denominator for tx_threshold windows
    ...
    model = keras.models.Sequential([
        layers.Flatten(input_shape=(1, 11)),
    ...
    while run == 1:
        ...
        hypot = [(glove_pos[0] - brush_corner[0]), (glove_pos[1] - brush_corner[1])]
        ...
        # try:
        #
        #     step_0 = int(hypot[0] / hypot[1])
        #     step_1 = int(hypot[1] / hypot[0])
        #     step_ratio = step_0 / step_1
        #
        # except:
        #
        #     step_0 = 0
        ...
        if pause == 0:
            ...
            for v in stream_direction:

                x_diff = 0
                y_diff = 0

                if v == 0:
                    x_diff = brush_corner[0]
                    y_diff = brush_corner[1] + int(brush_height / 2)

                if v == 1:
                    x_diff = brush_corner[1]
                    y_diff = CELL_WIDTH_0 - brush_corner[0] - int(brush_width / 2)

                if v == 2:
                    x_diff = CELL_WIDTH_0 - brush_corner[0] - brush_width
                    y_diff = CELL_HEIGHT_0 - brush_corner[1] - int(brush_height / 2)

                if v == 3:
                    x_diff = CELL_HEIGHT_0 - brush_corner[1] - brush_height
                ...
                for g in range(int(ev_3 / 4) + 1):
                    ...
                    for x in range(brush_width):
                        ...
                        cells_a[(0 + momentum[v] + y_diff + g) % (len(cells_a[:, 0])), (x + x_diff) % (len(cells_a[0, :]))] = value_color[d_rule[tuple(viewer_1d(cells_a[(1 + y_diff + g) % (len(cells_a[:, 0]))], (x + x_diff) % len(cells_a[0, :]), view, [], color_value))]]


                cells_a = np.rot90(cells_a, 4 - v, (0, 1))
                            ...
                            if len(input_list) > 1:
                                ...
                                try:
                                    ...
                                    if input_list[0] == 'h-w':
                                        ...
                                        CELL_HEIGHT = int(input_list[1])
                                        CELL_WIDTH = int(input_list[2])
                ...
                if event.type in [pygame.midi.MIDIIN]:
                    ...
                    if ev[1] == 1:
                        ...
                        if roller == 0:
                            ...
                            if ev_1 > sr_threshold:
                                ...
                                stream_direction.append(3)
                                stream_vel[3] = int(((ev_1 - (sr_threshold % 64)) % 64) / stream_vel_scale) + 1

                                if ev_7 > stutter_threshold:

                                    stream_direction.append(1)
                                    stream_vel[1] = int(((ev_1 - (sr_threshold % 64)) % 64) / stream_vel_scale) + 1

                                # print("right")
                                # print(stream_direction)

                            elif ev_1 < sl_threshold:

                                stream_direction.append(1)
                                stream_vel[1] = int((sl_threshold - ev_1) / stream_vel_scale) + 1

                                if ev_7 > stutter_threshold:

                                    stream_direction.append(3)
                                    stream_vel[3] = int((sl_threshold - ev_1) / stream_vel_scale) + 1
                        ...
                        # brush_corner[0] = int(CELL_WIDTH_0 / 127) * ev_1
                        ...
                        glove_pos[0] = ev_1 * (int(CELL_WIDTH_0 / 127) + 1)
                    ...
                    if ev[1] == 2:
                        ...
                        if roller == 0:
                            ...
                            if ev_2 > su_threshold:
                                ...
                                stream_direction.append(2)
                                stream_vel[2] = int(((ev_2 - (su_threshold % 64)) % 64) / stream_vel_scale) + 1

                                if ev_7 > stutter_threshold:

                                    stream_direction.append(0)
                                    stream_vel[0] = int(((ev_2 - (su_threshold % 64)) % 64) / stream_vel_scale) + 1

                                # print("up")
                                # print(stream_direction)

                            elif ev_2 < sd_threshold:

                                stream_direction.append(0)
                                stream_vel[0] = int((sd_threshold - ev_2) / stream_vel_scale) + 1

                                if ev_7 > stutter_threshold:

                                    stream_direction.append(2)
                                    stream_vel[2] = int((sd_threshold - ev_2) / stream_vel_scale) + 1
                        ...
                        # brush_corner[1] = CELL_HEIGHT_0 - int(CELL_HEIGHT_0 / 127) * ev_2  - 32
                        ...
                        glove_pos[1] = CELL_HEIGHT_0 - ev_2 * (int(CELL_WIDTH_0 / 127) + 1)
                    ...
                    if ev[1] == 4:
                        ...
                        if roller == 1:
                            ...
                            CELL_HEIGHT = int(cell_rows / 127) * ev_4
                            CELL_WIDTH = int(cell_row_width / 127) * ev_4


                        brush_width = int(brush_width_scale * ev_4 / brush_scale)
                        brush_height = int(brush_height_scale * ev_4 / brush_scale)

                        if brush_width < brush_min:

                            brush_width = brush_min

                        if brush_height < brush_min:

                            brush_height = brush_min
                        ...
                        if words_g != 3:
                            ...
                            if ev[2] > t1_threshold:
                                ...
                                if trigger_1 < 0:
                                    ...
                                    trigger_1 += int((int(((ev_8 - (t1_threshold % 64)) % 64) / trigger_scale) + 1) * tplus_scale)
                            ...
                            if ev[2] < t5_threshold:
                                ...
                                if trigger_5 < 0:
                                    ...
                                    trigger_5 += int((int((t5_threshold - ev_8) / trigger_scale) + 1) * tplus_scale)
                        ...
                        if words_g != 3:
                            ...
                            if ev[2] > t2_threshold:
                                ...
                                if trigger_2 < 0:
                                    ...
                                    trigger_2 += int((int(((ev_9 - (t2_threshold % 64)) % 64) / trigger_scale) + 1) * tplus_scale)
                            ...
                            if ev[2] < t6_threshold:
                                ...
                                if trigger_6 < 0:
                                    ...
                                    trigger_6 += int((int((t6_threshold - ev_9) / trigger_scale) + 1) * tplus_scale)
                        ...
                        if words_g != 3:
                            ...
                            if ev[2] > t3_threshold:
                                ...
                                if trigger_3 < 0:
                                    ...
                                    trigger_3 += int((int(((ev_10 - (t3_threshold % 64)) % 64) / trigger_scale) + 1) * tplus_scale)
                            ...
                            if ev[2] < t7_threshold:
                                ...
                                if trigger_7 < 0:
                                    ...
                                    trigger_7 += int((int((t7_threshold - ev_10) / trigger_scale) + 1) * tplus_scale)
                        ...
                        if words_g != 3:
                            ...
                            if ev[2] > t4_threshold:
                                ...
                                if trigger_4 < 0:
                                    ...
                                    trigger_4 += int((int(((ev_11 - (t4_threshold % 64)) % 64) / trigger_scale) + 1) * tplus_scale)
                            ...
                            if ev[2] < t8_threshold:
                                ...
                                if trigger_8 < 0:
                                    ...
                                    trigger_8 += int((int((t8_threshold - ev_11) / trigger_scale) + 1) * tplus_scale)
