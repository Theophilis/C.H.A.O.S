...
def rule_gen_fold(rule, base, length):
    rules = dict()
...
WHITE_PIXEL_2 = pygame.image.load(os.path.join('assets', 'magenta-' + str(value) + '.png')).convert()
...
GREY_PIXEL_X = pygame.image.load(os.path.join('assets', 'white-' + str(color_x_size) + '.png')).convert()
...
def Chaos_Window(base, pixel_res, cell_vel, device_id=None):
    ...
    def redraw_window(input_box, v_input, zero_count, step_show, triggers):
        ...
        #backround
        # backround = pygame.Rect(CELL_WIDTH, 0, CELL_WIDTH, HEIGHT)
        # pygame.draw.rect(WIN, (255, 255, 255), backround)
        ...
        if ui_on == 1:
            ...
            for x in range(ir_height):
                ...
                for v in ir:
                    ...
                    if v == 0:
                        cell = pygame.Rect(1 * color_x_size * clunk + x_offset, 1 * color_x_size + y_offset, pixel_res,
                                           pixel_res)
                    ...
                    if gv_track == tracker:
                        ...
                        pygame.draw.circle(WIN, (0, 0, 0), (1 * color_x_size * clunk + x_offset + color_x_size / 2, 1 * color_x_size + y_offset + color_x_size / 2), color_x_size / 2)
                        pygame.draw.circle(WIN, (255, 255, 255), (
                        1 * color_x_size * clunk + x_offset + color_x_size / 2,
                        1 * color_x_size + y_offset + color_x_size / 2), color_x_size / 2 - 1)
            ...
            for v in list(d_rule.keys())[tracker]:
                ...
                x_offset = CELL_WIDTH + 50
                y_offset = 200 + color_x_size * (ir_height + 1)
        ...
        if gaol == 1:
            ...
            gaol_w = main_font.render('gaol', 1, (255, 255, 255))
            WIN.blit(gaol_w, (CELL_WIDTH + 50, 130))

        draw_text('rule: ' + str(decimal(i_rule, base)), main_font, (255, 255, 255), WIN, CELL_WIDTH + 50, 10)
        draw_text('goal: ' + str(goal), main_font, (255, 255, 255), WIN, CELL_WIDTH + 50, 50)
        draw_text(str(decimal(row.tolist()[0], base)), main_font, (255, 255, 255), WIN, CELL_WIDTH + 50, 100)
    ...
    def mitosis(i, r, color, row, pixel_res):
        ...
        if r > 0:
            ...
            if base == 2:
                ...
                if pixel_res > 10:
                    if row[0, i] == 0:
                        cell = Cell(1 * pixel_res * i, 0, 'black_x')
                        cells[r].append(cell)
                    if row[0, i] == 1:
                        cell = Cell(1 * pixel_res * i, 0, 'blue_x')
                        cells[r].append(cell)
    ...
    #window
    cell_width = 32
    CELL_WIDTH = cell_width * pixel_res
    ...
    row_cast = np.asarray(rule_gen_fold(start, base, cell_row_width)[1])
    row = np.zeros((1, cell_row_width), dtype='int8')

    for x in range(len(row_cast)):
    ...
    while run == 1:
        ...
        for r in range(cell_rows):
            ...
            if len(cells[r]) == 0:
                ...
                row_history.append((row, len(row_history),r))
        ...
        row = np.asarray(row_history[-1][0])
