...
def map(length, width, rule, base, start, direction, cell_patterns, path, rc=0, plot=0):
    ...
    rule_call_f = rule_call
    rule_call_f = np.flip(rule_call_f, 0)
    # cnvs = np.flip(cnvs, 1)
...
#export pcolormesh of cells
    ...
    if plot !=0:
        if rc == 0:
            ...
            file = 'RC' + "-" + str(width) + '-' + str(rule) + '-' + str(base) + '-' + "X" + str(length)
            path_name = os.path.join(path, file)
...
for x in range(1, 256):
    ...
    rule_class = map(length, width, x, base, start, direction, cell_patterns, path, 1)

    print(rule_class)
    # print("")
    # print(cell_patterns[(width, x)])
    # print(len(cell_patterns[(width, x)]))
    # print((width, x))

# print(" ")


# c_i_s = chaos_mesh(cell_patterns, length, width, 255)
# print(c_i_s)
#
# cell_patterns = dict()
...
#
#
#     m = map(length, width, key[1], base, start, direction, cell_patterns, path, 1, 1)
